...
export default async function ProfilePage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: { followers: true, following: true, badges: { include: { badge: true } }, wallets: true }
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Star className="text-yellow-400" /> Points</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{user.points}</CardContent></Card>
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Users className="text-blue-400" /> Followers</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{user.followers.length}</CardContent></Card>
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Award className="text-purple-400" /> Badges</CardTitle></CardHeader><CardContent className="text-2xl font-bold">{user.badges.length}</CardContent></Card>
      </div>
